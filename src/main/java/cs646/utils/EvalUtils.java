...
public class EvalUtils {
	...
	public static Map<String, Map<String,Double>> loadQrels_score(  String f ) throws IOException {
		Map<String, Map<String,Double>> qrels_score = new TreeMap<>();
		BufferedReader reader = new BufferedReader( new InputStreamReader( new FileInputStream( new File(f) ), "UTF-8" ) );
		String line;
		while ( ( line = reader.readLine() ) != null ) {
			String[] splits = line.split( "\\s+" );
			String qid = splits[0];
			String docno = splits[2];
			qrels_score.putIfAbsent( qid, new HashMap<>());
			if ( Integer.parseInt( splits[3] ) == 1 ) {
				qrels_score.get( qid ).put( docno, 1.0 );
			}
			else if( Integer.parseInt( splits[3] ) == 2 ) {
				qrels_score.get( qid ).put( docno, 2.0 );
			}
		}
		reader.close();
		return qrels_score;
